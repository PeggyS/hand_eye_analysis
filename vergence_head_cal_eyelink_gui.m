function varargout = vergence_head_cal_eyelink_gui(varargin)
% VERGENCE_HEAD_CAL_EYELINK_GUI MATLAB code for vergence_head_cal_eyelink_gui.fig
%      VERGENCE_HEAD_CAL_EYELINK_GUI, by itself, creates a new VERGENCE_HEAD_CAL_EYELINK_GUI or raises the existing
%      singleton*.
%
%      H = VERGENCE_HEAD_CAL_EYELINK_GUI returns the handle to a new VERGENCE_HEAD_CAL_EYELINK_GUI or the handle to
%      the existing singleton*.
%
%      VERGENCE_HEAD_CAL_EYELINK_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VERGENCE_HEAD_CAL_EYELINK_GUI.M with the given input arguments.
%
%      VERGENCE_HEAD_CAL_EYELINK_GUI('Property','Value',...) creates a new VERGENCE_HEAD_CAL_EYELINK_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vergence_head_cal_eyelink_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vergence_head_cal_eyelink_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vergence_head_cal_eyelink_gui

% Last Modified by GUIDE v2.5 24-Oct-2020 16:54:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
   'gui_Singleton',  gui_Singleton, ...
   'gui_OpeningFcn', @vergence_head_cal_eyelink_gui_OpeningFcn, ...
   'gui_OutputFcn',  @vergence_head_cal_eyelink_gui_OutputFcn, ...
   'gui_LayoutFcn',  [] , ...
   'gui_Callback',   []);
...
% --- Executes just before vergence_head_cal_eyelink_gui is made visible.
function vergence_head_cal_eyelink_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vergence_head_cal_eyelink_gui (see VARARGIN)

% Choose default command line output for vergence_head_cal_eyelink_gui
handles.output = hObject;

% read in data files
disp('Choose horizontal head calibration eyelink *.bin file')
[fnSave, pnSave] = uigetfile({'*.bin'}, 'Choose *.bin file ...');
if isequal(fnSave,0) || isequal(pnSave,0)
   disp('no  file chosen ... ')
   return
end

handles.bin_filename = fullfile(pnSave, fnSave); 
...
if ~isempty(handles.eye_data.hh.pos)
	handles.line_hh = line(t, handles.eye_data.hh.pos, 'Tag', 'line_hh', 'Color', [0 .8 0]);
end
if ~isempty(handles.eye_data.hv.pos)
	handles.line_hv = line(t, handles.eye_data.hv.pos, 'Tag', 'line_hv', 'Color', [0 .8 0], ...
		'LineStyle', '--');
end
xlabel('Time (s)')
ylabel('Units')
...
function varargout = vergence_head_cal_eyelink_gui_OutputFcn(hObject, eventdata, handles)
