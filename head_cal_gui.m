function varargout = head_cal_gui(varargin)
% HEAD_CAL_GUI MATLAB code for head_cal_gui.fig
%      HEAD_CAL_GUI, by itself, creates a new HEAD_CAL_GUI or raises the existing
%      singleton*.
%
%      H = HEAD_CAL_GUI returns the handle to a new HEAD_CAL_GUI or the handle to
%      the existing singleton*.
%
%      HEAD_CAL_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HEAD_CAL_GUI.M with the given input arguments.
%
%      HEAD_CAL_GUI('Property','Value',...) creates a new HEAD_CAL_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before head_cal_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to head_cal_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help head_cal_gui

% Last Modified by GUIDE v2.5 14-Aug-2017 13:17:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
   'gui_Singleton',  gui_Singleton, ...
   'gui_OpeningFcn', @head_cal_gui_OpeningFcn, ...
   'gui_OutputFcn',  @head_cal_gui_OutputFcn, ...
   'gui_LayoutFcn',  [] , ...
   'gui_Callback',   []);
...
% --- Executes just before head_cal_gui is made visible.
function head_cal_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to head_cal_gui (see VARARGIN)

% Choose default command line output for head_cal_gui
handles.output = hObject;


% read in data filesdisp('Choose eye data *.bin file')
% eye
[fnSave, pnSave] = uigetfile({'*.bin'; '*.mat'}, 'Choose eye data *.bin or *.mat file ...');
...
if strcmp(ext, '.mat')
	handles = restore_state(handles, fullfile(pnSave, fnSave));
else
...
if isfield(handles, 'restore_data')
    updateEdTime(handles, handles.restore_data.scrub_line_eye.XData);
else
...
% video scrub line in the data plot
x_scrub_line = 1/samp_freq;
axes(handles.axes_head)
handles.scrub_line_eye = line( [x_scrub_line, x_scrub_line], handles.axes_head.YLim, ...
...
function varargout = head_cal_gui_OutputFcn(hObject, eventdata, handles)
...
head_cal.cal_bin_filename = handles.bin_filename;
head_cal.cal_hdf_filename = handles.hdf_filename;
head_cal.cal_vid_filename = handles.vid_filename;
