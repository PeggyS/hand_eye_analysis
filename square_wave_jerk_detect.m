	...
	for source_num = 1:length(eye_data.(eye).saccades)
		if ~isfield(eye_data.(eye).saccades(source_num).sacclist, 'swj')
			if isfield(eye_data.(eye).saccades(source_num).sacclist, 'start')
				startIndex = round((eye_data.(eye).saccades.sacclist.start - eye_data.start_times) / 1000 * eye_data.samp_freq);
				endIndex = round((eye_data.(eye).saccades.sacclist.end - eye_data.start_times) / 1000 * eye_data.samp_freq);
				amplitude = abs(eye_data.(eye).saccades.sacclist.endpos - eye_data.(eye).saccades.sacclist.startpos);
				% direction = angle betw 0 & 360, pure vertical = 0, 180,
				% horizontal = 90, 270
				if strcmp(eye_data.(eye).saccades.paramtype, 'findsaccs')
					if contains(eye, 'h')
						direction = sign(eye_data.(eye).saccades.sacclist.endpos - eye_data.(eye).saccades.sacclist.startpos);
					...
					if strcmp(eye, 'h')
						dx = eye_data.(eye).saccades.sacclist.endpos - eye_data.(eye).saccades.sacclist.startpos;
						dy = eye_data.(eye_other_direction).saccades.sacclist.endpos - eye_data.(eye_other_direction).saccades.sacclist.startpos;
					else
						dy = eye_data.(eye).saccades.sacclist.endpos - eye_data.(eye).saccades.sacclist.startpos;
						dx = eye_data.(eye_other_direction).saccades.sacclist.endpos - eye_data.(eye_other_direction).saccades.sacclist.startpos;
					end
