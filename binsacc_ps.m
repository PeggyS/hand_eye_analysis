function sac = binsacc_ps(sacl,sacr)
%--------------------------------------------------------------------
%  FUNCTION binsacc.m
...
for i=1:NL              % loop over left-eye saccades                  
    ...
    if NR>0
        ...
        if ~isempty(overlap)
            ...
            % I disagree about using the mean of l & r data - ps
%             s = [min([l1 r1]) max([l2 r2]) ...
%                 mean([vl vr]) mean([ampl ampr]) ...
%                 phi mean([dxl dxr]) mean([dyl dyr])];
            % the eye of interest is sent as the first parameter (sacl)
            s = sacl(i,:);
...
% NO. I refuse to impose this restriction without taking the sampling
% frequency into account! - ps
% nsac = size(sac,1);
% k = 1;
% while k<nsac
